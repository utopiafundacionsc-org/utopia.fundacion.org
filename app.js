...
(() => {
  ...
  const BUILD_VERSION = "12.6.4-20260717";
  ...
  async function postForm(payload, timeoutMs = 90000) {
    ...
    try {
      ...
      if (error?.name === "AbortError") throw new Error("El envío tardó demasiado. Verifica tu conexión y vuelve a presionar el botón.");
      throw new Error("No fue posible conectar con Google Apps Script.");
    } finally {
      if (timeout) window.clearTimeout(timeout);
    }
  ...
  function selectedPaymentMethod() {
    const selectedId = String(qs("#paymentMethod")?.value || state.reservation?.paymentMethod || "");
    const existing = state.paymentMethods.find((method) => method.id === selectedId);
    if (existing) return existing;
    if (state.reservation?.paymentType) {
      return normalizePaymentMethod({
        id: selectedId || state.reservation.paymentType,
        type: state.reservation.paymentType,
        label: state.reservation.paymentLabel || state.reservation.paymentType,
        enabled: true
      });
    }
    return null;
  ...
  function syncFormAvailability() {
    ...
    const hasConfirmedReservation = Boolean(state.reservation?.code);
    const method = selectedPaymentMethod();
    const type = method?.type || state.reservation?.paymentType || "";
    const canShowPayment = hasConfirmedReservation || (Boolean(method) && isTermsAccepted() && state.selected.size > 0);
    ...
    /*
      El botón no debe quedar bloqueado por una carrera entre la reserva
      automática y la selección del comprobante. Las validaciones completas
      se ejecutan al enviar; aquí solo exigimos que el pago esté visible y que
      el comprobante requerido ya haya sido elegido.
    */
    /*
      El botón permanece disponible cuando ya se muestra el método de pago.
      Las validaciones se presentan al presionarlo, en lugar de dejarlo
      bloqueado silenciosamente por una carrera de la reserva automática.
    */
    const canAttemptSubmit = canShowPayment && !state.isSubmitting;
    const submit = qs("#submitButton");
    if (submit) {
      submit.disabled = !canAttemptSubmit;
      submit.setAttribute("aria-disabled", String(!canAttemptSubmit));
      submit.title = canAttemptSubmit
        ? (proofReady ? "Registrar participación y enviar comprobante" : "Presiona para revisar qué dato falta")
        : (state.isSubmitting ? "Enviando participación" : "Selecciona números, acepta los términos y elige el método de pago");
    }
  }
  async function reserveTickets() {
    if (state.reservation?.code) return state.reservation;
    if (state.reservationPromise) return state.reservationPromise;

    const person = validateParticipantFields();
    state.reservationPromise = (async () => {
      ...
      try {
        ...
        const message=qs("#reservationMessage");
        if(message){message.hidden=false;message.textContent="Reserva confirmada. Completa el pago y envía tu comprobante antes de que termine el tiempo.";}
    ...
    try {
      return await state.reservationPromise;
    } finally {
  ...
  function loadImageFromDataUrl(dataUrl) {
    return new Promise((resolve, reject) => {
      ...
      image.onerror = () => reject(new Error("No se pudo procesar la imagen del comprobante."));
      image.src = dataUrl;
    });
  }

  async function optimizeImageProof(file, mime) {
    if (file.size <= 1500 * 1024) return {blob:file, mime, name:file.name};
    try {
      ...
      if (!context) return {blob:file, mime, name:file.name};
      ...
      return {blob, mime:"image/jpeg", name:`${baseName}.jpg`};
    } catch (error) {
      console.warn("No se pudo optimizar el comprobante; se enviará el archivo original.", error);
      return {blob:file, mime, name:file.name};
    }
  }

  async function readProof(file) {
    if (!file) throw new Error("Adjunta el comprobante de pago.");
    const extension = String(file.name || "").split(".").pop().toLowerCase();
    const mimeByExtension = {jpg:"image/jpeg",jpeg:"image/jpeg",png:"image/png",webp:"image/webp",pdf:"application/pdf"};
    const mime = String(file.type || mimeByExtension[extension] || "").toLowerCase();
    const allowed = ["image/jpeg", "image/png", "image/webp", "application/pdf"];
    if (!allowed.includes(mime)) throw new Error("El comprobante debe ser JPG, PNG, WEBP o PDF.");
    if (file.size > MAX_PROOF_BYTES) throw new Error("El comprobante supera 5 MB.");

    const prepared = mime === "application/pdf" ? {blob:file,mime,name:file.name} : await optimizeImageProof(file,mime);
    if (prepared.blob.size > MAX_PROOF_BYTES) throw new Error("El comprobante supera 5 MB incluso después de optimizarlo.");
    const dataUrl = await fileToDataUrl(prepared.blob);
    return {
      proofName: prepared.name,
      proofMime: prepared.mime,
  ...
  async function submitParticipation(event) {
    ...
    result.textContent = "";
    result.style.color = "";
    setSubmissionStatus("Validando los datos del participante…", "info");
    ...
    try {
      ...
      setSubmissionStatus(proofRequired ? "Preparando el comprobante…" : "Preparando el registro…", "info");
      const proofData = proofRequired ? await readProof(proofFile) : {};
      state.proofFile = proofFile || null;

      setSubmissionStatus("Confirmando la reserva de tus números…", "info");
      if (!state.reservation?.code) await reserveTickets();
      if (!state.reservation?.code) throw new Error("No se pudo confirmar la reserva. Vuelve a presionar el botón.");

      setSubmissionStatus("Enviando el comprobante. No cierres esta página…", "info");
      const data = await postForm({action:"confirmRegistration",raffleId:currentRaffleId(),participationCode:state.reservation.code,...proofData}, 120000);
      ...
      setSubmissionStatus("Participación registrada correctamente.", "success");
      ...
      state.reservation = null; state.reservationPromise = null; state.hold = null; state.holdTickets.clear(); state.holdSignature = "";
      const paymentSelect=qs("#paymentMethod");if(paymentSelect)paymentSelect.disabled=false;
      if (state.reservationTimer) window.clearInterval(state.reservationTimer);
      state.selected.clear();
      ...
      console.error(error);
      const message = publicErrorMessage(error,"No se pudo completar el registro en este momento. Intenta nuevamente.");
      setSubmissionStatus(message, "error");
      result.style.color = "#b42318";
      result.textContent = message;
      qs("#submissionStatus")?.scrollIntoView({behavior:"smooth",block:"center"});
    } finally {
  ...
  function applyWinnersData(data) {
    ...
    winners.sort((a, b) => Number(a.order) - Number(b.order)).forEach((winner) => {
      const article = document.createElement("article");
      article.className = "winner-card winner-card-simple";
      article.innerHTML = `
        <span class="winner-card-prize-order">PREMIO ${escapeHtml(winner.order || "")}</span>
        <h3 class="winner-card-prize-name">${escapeHtml(winner.prizeName || "Premio")}</h3>
        <span class="winner-card-label">TICKET GANADOR</span>
        <div class="winner-number">${escapeHtml(winner.ticket)}</div>
        <span class="winner-card-name-label">GANADOR</span>
        <h3 class="winner-card-person-name">${escapeHtml(winner.fullName || "Titular no registrado")}</h3>`;
  ...
  function setupEvents() {
    ...
    on("#submitButton", "click", submitParticipation);

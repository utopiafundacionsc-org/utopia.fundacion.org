...
(() => {
  ...
  async function loadPublicRaffles() {
    ...
    const requested = REQUESTED_RAFFLE_ID;
  ...
  async function syncTicketHold() {
    if (state.reservation || state.isHolding) return;
    ...
    try {
      ...
      state.board = state.board.map((item) => state.selected.has(item.number) ? {...item,status:"RESERVADO",label:"Reservado por ti"} : item);
      updateSelection();
      renderBoard();
      await loadBoard();
    } finally {
  ...
  async function reserveTickets() {
    ...
    if (!state.hold) await syncTicketHold();
    ...
    try {
      ...
      qs("#reservationMessage").textContent = `Reserva confirmada. Código: ${data.participationCode}`;
  ...
  async function submitParticipation(event) {
    ...
    try {
      ...
      state.reservation = null; state.hold = null;
      const paymentSelect=qs("#paymentMethod");if(paymentSelect)paymentSelect.disabled=false;
      if (state.reservationTimer) window.clearInterval(state.reservationTimer);
      state.selected.clear();
      ...
      window.setTimeout(() => { shareWhatsapp().catch(() => {}); }, 350);
    } catch (error) {
      console.error(error);
  ...
  function setupEvents() {
    ...
    on("#printReceipt", "click", () => {const receipt=qs("#digitalReceipt");if(!receipt)return;receipt.hidden=false;window.print();receipt.hidden=true;});
  ...
  async function initialize() {
    ...
    try {
      await loadPublicRaffles();
      await loadRaffle();
      await Promise.all([loadBoard({preservePage:false}),loadWinners()]);
    } catch (error) {
      console.error(error);
      setPublicNotice(error.message,"error");
    }

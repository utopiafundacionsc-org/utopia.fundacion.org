...
(() => {
  ...
  function renderPrizes(prizes = []) {
    ...
    if (!grid) return;
    const source = [...prizes].sort((a,b) => Number(a.order || 0) - Number(b.order || 0));
    ...
    if (!source.length) {
      grid.innerHTML = '<article class="empty-card"><h3>Premios por publicar</h3><p>Los premios aparecerán cuando sean configurados.</p></article>';
      return;
    }
  ...
  async function loadBoard({preservePage = true} = {}) {
    ...
    state.board = normalizeBoard(data.tickets || []);
    ...
    const stats = {
      total: Number(baseStats.total ?? baseStats.totalTickets ?? baseStats.totalNumeros ?? calculatedStats.total),
      sold: Number(baseStats.sold ?? baseStats.vendidos ?? baseStats.approved ?? calculatedStats.sold),
      reserved: Number(baseStats.reserved ?? baseStats.reservados ?? baseStats.pending ?? calculatedStats.reserved),
      review: Number(baseStats.review ?? baseStats.enRevision ?? baseStats.en_revision ?? calculatedStats.review),
      available: Number(baseStats.available ?? baseStats.disponibles ?? calculatedStats.available),
      blocked: Number(baseStats.blocked ?? baseStats.bloqueados ?? calculatedStats.blocked)
    };
  ...
  function setupEvents() {
    ...
    qs("#terms").addEventListener("change", syncFormAvailability);
    ...
    qs("#paymentMethod").addEventListener("change", async () => {
      syncFormAvailability();
    ...
    qs("#paymentProof").addEventListener("change", () => {
      ...
      qs("#proofFileName").textContent = file
  ...
  async function initialize() {
    ...
    try {
      ...
      console.error(error);
      const formResult = qs("#formResult");

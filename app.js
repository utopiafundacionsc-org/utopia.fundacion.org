...
(() => {
  ...
  const BUILD_VERSION = "12.5.7-20260717";
  ...
  function syncFormAvailability() {
    ...
    */
    const canSubmit = canShowPayment && proofReady && !state.isReserving && !state.isSubmitting;
    const submit = qs("#submitButton");
    if (submit) {
      submit.disabled = !canSubmit;
      submit.setAttribute("aria-disabled", String(!canSubmit));
      submit.title = canSubmit
        ? "Enviar participación y comprobante"
        : (state.isReserving ? "Espera mientras se confirma la reserva" : (proofRequired && !proofReady ? "Adjunta el comprobante para continuar" : "Completa los datos para continuar"));
    }
  }
  async function reserveTickets() {
    ...
    try {
      ...
      /* Recalcular después de terminar la reserva: antes se hacía mientras
         isReserving seguía en true y el botón podía quedar bloqueado. */
      window.setTimeout(syncFormAvailability, 0);
    }
  }
  ...
  async function submitParticipation(event) {
    ...
    try {
      ...
      if (!state.reservation?.code) throw new Error("No se pudo confirmar la reserva. Presiona nuevamente el botón para reintentar.");

  ...
  (()=>{
    ...
    const BUILD_VERSION="12.6.2-20260717";
    ...
    function startLiveAnimation(){
      ...
      const timerId=setInterval(()=>{
        if(state.liveSpinTimer!==timerId)return;
        state.liveDigits=randomLiveTicketDigits();
        renderLiveReels();
      },60);
      state.liveSpinTimer=timerId;
    }
    ...
    function renderLiveWinners(winners){
      ...
      const card=w=>`<article class="live-winner-card winner-number-only"><img alt="${esc(w.prizeName)}" data-image-source="${esc(w.prizeImageUrl||'')}" data-prize-id="${esc(w.prizeId||'')}" data-fallback="${esc(prizeFallback({order:w.prizeOrder}))}" src="${esc(safeImageUrl(w.prizeImageUrl,prizeFallback({order:w.prizeOrder})))}"/><div class="meta"><small>Premio ${esc(w.prizeOrder)}</small><strong>${esc(w.prizeName)}</strong><span class="ticket">Ticket ganador: ${esc(w.ticket)}</span></div></article>`;
...
function updateLivePrizePreview(prizeId=''){
  const available=state.liveAvailablePrizes||[];
  const allPrizes=(state.liveSetup?.prizes||state.liveSetup?.raffle?.prizes||[]);
  const resolvedPrize=(available.find(p=>String(p.id)===String(prizeId))||allPrizes.find(p=>String(p.id)===String(prizeId))||null);
  const fallbackWinner=(!prizeId&&state.liveCurrentWinner)?state.liveCurrentWinner:null;
  const prize=resolvedPrize||fallbackWinner;
  const img=qs('#livePrizePreviewImage',false),name=qs('#livePrizePreviewName',false);
  if(img)setResilientImage(img,prize?.imageUrl||prize?.prizeImageUrl||'',prize?(prizeFallback(prize,Number(prize?.order||prize?.prizeOrder||0)-1)):'assets/rifa-solidaria-2026.png',prize?.id||prize?.prizeId||'',prize?.name||prize?.prizeName||'Fotografía del premio');
  if(name){
    if(prize){
      const order=prize.order||prize.prizeOrder||'';
      const prizeName=prize.name||prize.prizeName||'Premio';
      name.textContent=`${order}. ${prizeName}`.replace(/^\. /,'').trim();
    }else{
      name.textContent='Fotografía del premio';
    }
  }
}
function renderLivePrizeOptions(prizes,keep=''){
      const all=(prizes||[]);
      const available=all.filter(p=>!p.drawn&&p.status!=='DESACTIVADO');
      const select=qs('#livePrizeSelect');
      state.liveAvailablePrizes=available;
      const lastDrawn=state.liveCurrentWinner;
      const placeholder=(lastDrawn&&(!keep||!available.some(p=>String(p.id)===String(keep))))
        ? `Último premio sorteado: ${(lastDrawn.prizeOrder||'')}. ${lastDrawn.prizeName||'Premio'}`.replace(/^Último premio sorteado: \. /,'Último premio sorteado: ')
        : 'Seleccionar premio';
      select.innerHTML=`<option value="">${esc(placeholder)}</option>`+available.map(p=>`<option value="${esc(p.id)}">${esc(p.order)}. ${esc(p.name)}</option>`).join('');
      ...
      qs('#liveStartDraw').disabled=!state.liveMigrationReady||!select.value||Boolean(state.liveSession)||Boolean(state.liveStarting);
      updateLivePrizePreview(select.value||'');
      updateLiveBurnedVisibility();
      syncLivePrimaryButton();
    }
    function currentLiveWinnerAt(){
      return Math.max(1,Math.floor(Number(state.liveSession?.winnerAt||qs('#liveWinnerAt',false)?.value||1)));
    }
    function updateLiveBurnedVisibility(){
      const hide=currentLiveWinnerAt()<=1;
      const panel=qs('#liveBurnedPanel',false);
      const stage=qs('#liveStage',false);
      if(panel)panel.hidden=hide;
      if(stage)stage.classList.toggle('single-winner-mode',hide);
      if(!hide)renderLiveBurned();
    }
    function setLiveWinnerVisual(active){
      const stage=qs('#liveStage',false);
      const card=qs('#liveWinnerCard',false);
      if(stage)stage.classList.toggle('has-current-winner',Boolean(active));
      if(card)card.hidden=!active;
    }
    function syncLivePrimaryButton(){
      const button=qs('#liveRevealDigit',false);
      if(!button)return;
      button.hidden=false;
      if(state.liveSession){
        button.disabled=Boolean(state.liveRevealPending);
        button.textContent=state.liveRevealPending?'DETENIENDO…':'DETENER';
        return;
      }
      const canStart=Boolean(state.liveMigrationReady&&qs('#livePrizeSelect',false)?.value);
      button.disabled=!canStart||Boolean(state.liveStarting);
      button.textContent=state.liveStarting?'INICIANDO…':(canStart?'INICIAR DINÁMICA':'SELECCIONA UN PREMIO');
    }
    async function handleLivePrimaryAction(){
      if(state.liveSession)return revealLiveDigit();
      return startLiveDraw({skipConfirm:state.liveRunConfirmedRaffleId===state.liveRaffleId});
    }

async function handleLivePrizeSelection(){
  const value=qs('#livePrizeSelect').value;
  qs('#liveStartDraw').disabled=!state.liveMigrationReady||!value||Boolean(state.liveSession)||Boolean(state.liveStarting);
  updateLivePrizePreview(value);
  const winnerCard=qs('#liveWinnerCard',false);
  if(value&&state.liveCurrentWinner&&winnerCard){
    setLiveWinnerVisual(false);
    qs('#liveProgress').textContent='Premio listo. Pulsa INICIAR DINÁMICA para continuar.';
  }else if(!value&&state.liveCurrentWinner){
    qs('#liveProgress').textContent='Ganador registrado. Selecciona el siguiente premio.';
  }
  syncLivePrimaryButton();
  updateLiveBurnedVisibility();
...
function ensureLiveVideoPlayback(){
    ...
    async function startLiveDraw(options={}){
      ...
      if(!skipConfirm&&state.liveRunConfirmedRaffleId!==state.liveRaffleId){
      ...
      try{
        ...
        updateLiveBurnedVisibility();
        ensureLiveVideoPlayback();
        qs('#liveProgress').textContent='Dinámica iniciada. Pulsa DETENER para fijar un número.';
...
async function revealLiveDigit(){
  if(!state.liveSession||state.liveRevealPending)return;
  const button=qs('#liveRevealDigit');
  const sessionToken=state.liveSession.token;
  state.liveRevealPending=true;
  button.disabled=true;
  button.textContent='DETENIENDO…';
  const stage=qs('#liveStage',false);
  if(stage)stage.classList.add('is-stopping');
  if(!state.liveSpinTimer)startLiveAnimation();
  const stoppingStarted=performance.now();
      try{
        const d=await api('adminLiveDrawReveal',{drawToken:sessionToken});
        const remainingAnimation=Math.max(0,900-(performance.now()-stoppingStarted));
        ...
        if(d.completed){
          ...
          button.textContent='SELECCIONA EL SIGUIENTE PREMIO';
          const winnerLabel=qs('#liveWinnerCard .winner-label',false); if(winnerLabel)winnerLabel.textContent='GANADOR';
          button.disabled=true;
          button.hidden=false;
          qs('#liveCancelDraw').disabled=true;
          const winnerTicket=qs('#liveWinnerTicket',false);if(winnerTicket)winnerTicket.textContent=String(w.ticket||d.ticket||'---');
          setLiveWinnerVisual(true);
          ...
          qs('#liveProgress').textContent='Ganador registrado. Selecciona el siguiente premio para continuar.';
          toast('¡Ganador registrado! El fondo y la configuración se mantienen.');
          ...
          for(let remaining=10;remaining>0;remaining--){
            ...
            qs('#liveProgress').textContent='Número al agua / quemado. Sigue participando en el siguiente intento.';
          ...
          qs('#liveProgress').textContent='Dinámica iniciada. Pulsa DETENER para fijar el siguiente número.';
        }
      }catch(e){
        ...
        state.liveRevealPending=false;
        const stage=qs('#liveStage',false);
        if(stage)stage.classList.remove('is-stopping');
        syncLivePrimaryButton();
      }
    }
    ...
    function setup(){
      ...
      bind("#liveRevealDigit","click",()=>handleLivePrimaryAction().catch(error=>toast(error.message,"error")));
      ...
      bind("#livePrizeSelect","change",()=>handleLivePrizeSelection().catch(error=>toast(error.message,"error")));
      bind("#liveWinnerAt","change",()=>{rememberLiveWinnerAt();updateLiveBurnedVisibility();syncLivePrimaryButton();});
      bind("#liveWinnerAt","blur",()=>{rememberLiveWinnerAt();updateLiveBurnedVisibility();});
      bind("#liveWinnerAt","input",()=>{updateLiveBurnedVisibility();syncLivePrimaryButton();});

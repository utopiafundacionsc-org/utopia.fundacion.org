  ...
  (()=>{
    ...
    const BUILD_VERSION="12.5.2-20260716";
    ...
    async function includeExpiredInDraw(code,button){if(!confirm('¿Incluir todos los tickets vencidos de esta participación en el sorteo? El estado de pago seguirá figurando como VENCIDO.'))return;button.disabled=true;try{const d=await api('adminIncludeExpiredInDraw',{participationCode:code});if(!d.ok)throw new Error(d.error);invalidateViews('dashboard','numbers','liveDraw','reports');toast(`${d.included?.length||0} ticket(s) incluidos en el sorteo.`);await loadParticipants();}finally{button.disabled=false}}

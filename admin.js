  ...
  (()=>{
    ...
    const state={token:localStorage.getItem("utopiaAdminToken")||"",user:null,raffles:[],raffleId:DEFAULT_RAFFLE,currentView:"dashboard",orders:[],participants:[],payments:[],board:[],filteredBoard:[],numberPage:0,selectedNumber:null,results:[],report:null,raffleData:null,publicBackgroundDraft:null,liveRaffleId:DEFAULT_RAFFLE,liveSetup:null,liveSession:null,liveDigits:[],liveBurned:[],liveSpinTimer:null,liveMigrationReady:false,liveBackgroundDraft:null,loadedAt:{},refreshSequence:0};
    ...
    const BUILD_VERSION="12.2.0-20260715";
    ...
    async function loadLiveDraw(preserveWinner=false){
      ...
      qs('#livePrepareBackground').disabled=!state.liveMigrationReady;qs('#liveApplyBackground').disabled=!state.liveMigrationReady||!state.liveBackgroundDraft;qs('#liveFullscreen').disabled=!state.liveMigrationReady;qs('#liveMigrationMessage').textContent=state.liveMigrationReady?'Tickets válidos listos. Cambia el fondo, amplía la pantalla y selecciona el premio.':'Migra los tickets válidos antes de continuar.';
    ...
    async function syncLiveTickets(){
      const button=qs('#liveSyncTickets');button.disabled=true;try{const d=await api('adminLiveDrawSync',{raffleId:state.liveRaffleId});if(!d.ok)throw new Error(d.error);state.liveSetup=d;state.liveMigrationReady=Boolean(d.migrationReady??((d.stats?.eligibleTickets||0)>0&&(d.stats?.approvedNotSold||0)===0));renderLiveStats(d.stats||{});renderLivePrizeOptions(d.prizes||[],'');renderLiveWinners(d.winners||[]);const m=d.migration||{};qs('#liveMigrationMessage').textContent=`Migración completada: ${m.synchronized||0} actualizados, ${m.alreadySold||0} ya estaban vendidos. ${state.liveMigrationReady?'Ya puedes cargar el fondo y continuar.':'No existen tickets elegibles todavía.'}`;qs('#livePrepareBackground').disabled=!state.liveMigrationReady;qs('#liveApplyBackground').disabled=!state.liveMigrationReady||!state.liveBackgroundDraft;qs('#liveFullscreen').disabled=!state.liveMigrationReady;toast('Tickets válidos sincronizados.');}finally{button.disabled=false;}
    }
    ...
    function setup(){
      ...
      bind("#liveRaffleSelect","change",e=>{state.liveRaffleId=e.target.value;state.liveSession=null;state.liveBackgroundDraft=null;qs('#liveBackgroundReady').hidden=true;qs('#liveApplyBackground').disabled=true;stopLiveAnimation();loadLiveDraw()});

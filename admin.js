  ...
  (()=>{
    ...
    const BUILD_VERSION="12.2.9-20260715";
    ...
    function renderLiveWinners(winners){
      ...
      const card=w=>`<article class="live-winner-card"><img alt="${esc(w.prizeName)}" src="${esc(w.prizeImageUrl||prizeFallback({order:w.prizeOrder}))}"/><div class="meta"><small>Premio ${esc(w.prizeOrder)}</small><strong>${esc(w.prizeName)}</strong><span class="ticket">${esc(w.ticket)}</span><p>${esc(w.fullName)}</p><p>WhatsApp: ${esc(w.maskedPhone||'No registrado')}</p><p>${esc(w.city||'')}</p><p>${esc(w.drawDate||'')}</p></div></article>`;
    ...
    function applyLiveBackground(url,type='IMAGEN'){
      ...
      if(mediaType==='VIDEO'&&url){
        ...
        video.onerror=()=>{console.warn('[Sorteos Utopía] No se pudo cargar el video del sorteo en vivo:',videoUrl);stage.dataset.bgType='image';stage.style.backgroundImage='linear-gradient(180deg,rgba(1,16,39,.55),rgba(1,28,57,.88)),url("assets/sorteos-en-vivo-utopia.png")';};
        stage.style.backgroundImage='linear-gradient(180deg,rgba(1,16,39,.26),rgba(1,28,57,.58))';
        tryPlay();
      } else {
        video.pause();
    ...
    async function revealLiveDigit(){
      ...
      try{
        ...
        if(d.completed){
          ...
          const prizeImg=qs('#liveWinnerPrizeImage',false);if(prizeImg)prizeImg.src=w.prizeImageUrl||prizeFallback({order:w.prizeOrder});
    ...
    async function nextLivePrize(){stopLiveAnimation();state.liveSession=null;state.liveCurrentWinner=null;state.liveBurned=[];lockLiveControls(false);qs('#liveWinnerCard').hidden=true;renderLiveWinners(state.liveWinners);await loadLiveDraw();toast('Listo para seleccionar el siguiente premio.');}

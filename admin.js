  ...
  (()=>{
    ...
    const state={token:localStorage.getItem("utopiaAdminToken")||"",user:null,raffles:[],raffleId:DEFAULT_RAFFLE,deletedReports:[],currentView:"dashboard",orders:[],participants:[],payments:[],board:[],filteredBoard:[],numberPage:0,selectedNumber:null,results:[],report:null,raffleData:null,publicBackgroundDraft:null,liveRaffleId:DEFAULT_RAFFLE,liveSetup:null,liveSession:null,liveDigits:[],liveBurned:[],liveSpinTimer:null,liveMigrationReady:false,liveBackgroundDraft:null,liveCurrentWinner:null,liveWinners:[],loadedAt:{},refreshSequence:0};
    ...
    const BUILD_VERSION="12.2.7-20260715";
    ...
    function setLiveStageBrand(organization){const brand=organizationBrand(organization);const logo=getElement('#liveStageOrgLogo',false);const nameEl=getElement('#liveStageOrgName',false);if(logo){logo.src=brand.logo;logo.alt=brand.alt;}if(nameEl)nameEl.textContent=brand.name;}
    const resolveStageVideoUrl=value=>{const text=String(value||'').trim();if(!text)return text;const id=driveFileId(text);return id?`https://drive.google.com/uc?export=view&id=${encodeURIComponent(id)}`:text;};
    ...
    async function loadRaffles(){
      ...
      if(!state.raffles.length){selector.innerHTML='<option value="">Sin sorteos disponibles</option>';const liveSelector=qs("#liveRaffleSelect");if(liveSelector)liveSelector.innerHTML='<option value="">Sin sorteos disponibles</option>';updatePublicPageLink();return;}
    ...
    function updatePublicPageLink(){
      ...
      if(!link)return;
      link.href=state.raffleId?`index.html?raffleId=${encodeURIComponent(state.raffleId)}`:"index.html";
    }
    ...
    function liveTicketRange(){
      const total=Math.max(1,Math.floor(Number(state.liveSetup?.raffle?.totalTickets||state.liveSession?.totalTickets||1)));
      const digitCount=Math.max(3,Math.floor(Number(state.liveSession?.digitCount||state.liveSetup?.raffle?.digitCount||String(total).length)));
      return {total,digitCount};
    }
    function randomLiveTicketDigits(){
      const {total,digitCount}=liveTicketRange();
      const ticket=1+Math.floor(Math.random()*total);
    ...
    function renderLiveWinners(winners){
      ...
      const card=w=>`<article class="live-winner-card"><small>Premio ${esc(w.prizeOrder)}</small><strong>${esc(w.prizeName)}</strong><span class="ticket">${esc(w.ticket)}</span><p>${esc(w.fullName)}</p><p>WhatsApp: ${esc(w.maskedPhone||'No registrado')}</p><p>${esc(w.city||'')}</p><p>${esc(w.drawDate||'')}</p></article>`;
      ...
      if(stageList){
        const current=state.liveCurrentWinner;
        const previous=current?list.filter(w=>!(String(w.ticket)===String(current.ticket)&&String(w.prizeName)===String(current.prizeName))):list;
        stageList.innerHTML=previous.length?previous.map(w=>`<article class="live-stage-winner-mini"><small>Premio ${esc(w.prizeOrder)}</small><strong>${esc(w.prizeName)}</strong><span>${esc(w.ticket)}</span><p>${esc(w.fullName)}</p><p>WhatsApp: ${esc(w.maskedPhone||'No registrado')}</p></article>`).join(''):'<div class="live-stage-winners-empty">Los ganadores aparecerán aquí.</div>';
      }
    }
    ...
    function applyLiveBackground(url,type='IMAGEN'){
      ...
      if(mediaType==='VIDEO'&&url){
        ...
        video.onerror=()=>{console.warn('[Sorteos Utopía] No se pudo cargar el video del sorteo en vivo:',videoUrl);};
        stage.style.backgroundImage='linear-gradient(180deg,rgba(1,16,39,.35),rgba(1,28,57,.72))';
      } else {
        video.pause();
        ...
        stage.style.backgroundImage=`linear-gradient(180deg,rgba(1,16,39,.55),rgba(1,28,57,.88)),url("${imageUrl}")`;
      }
    }
    async function prepareLiveBackground(){if(!state.liveMigrationReady)throw new Error('Primero migra los tickets válidos.');const file=qs('#liveBackgroundFile').files[0];if(!file)throw new Error('Selecciona una imagen o video.');const button=qs('#livePrepareBackground');button.disabled=true;qs('#liveApplyBackground').disabled=true;qs('#liveBackgroundReady').hidden=true;try{const uploaded=await uploadMedia(file);state.liveBackgroundDraft={url:uploaded.url,type:uploaded.type,name:file.name};applyLiveBackground(uploaded.url,uploaded.type);qs('#liveBackgroundReadyText').textContent=`${file.name} está listo para aplicar`;qs('#liveBackgroundReady').hidden=false;qs('#liveApplyBackground').disabled=false;toast('Fondo preparado. Revisa la vista y presiona Aplicar fondo.');}finally{button.disabled=false}}
    async function applyLiveBackgroundDraft(){if(!state.liveMigrationReady)throw new Error('Primero migra los tickets válidos.');if(!state.liveBackgroundDraft)throw new Error('Primero prepara una imagen o video.');const button=qs('#liveApplyBackground');button.disabled=true;try{const d=await api('adminSaveLiveBackground',{raffleId:state.liveRaffleId,liveBackgroundUrl:state.liveBackgroundDraft.url,liveBackgroundType:state.liveBackgroundDraft.type});if(!d.ok)throw new Error(d.error);applyLiveBackground(state.liveBackgroundDraft.url,state.liveBackgroundDraft.type);state.liveBackgroundDraft=null;qs('#liveBackgroundFile').value='';qs('#liveBackgroundReadyText').textContent='Fondo aplicado correctamente';qs('#liveBackgroundReady').hidden=false;invalidateViews('liveDraw','reports');state.loadedAt.liveDraw=Date.now();toast('Fondo del sorteo aplicado.');}catch(error){button.disabled=false;throw error}}
    async function loadLiveDraw(preserveWinner=false){
      ...
      const previousPrize=qs('#livePrizeSelect').value;const d=await api('adminLiveDrawSetup',{raffleId:state.liveRaffleId});if(!d.ok)throw new Error(d.error);if(!preserveWinner)state.liveCurrentWinner=null;state.liveSetup=d;state.liveMigrationReady=Boolean(d.migrationReady);qs('#liveRaffleTitle').textContent=d.raffle?.name||'Sorteo Utopía';setLiveStageBrand(d.raffle?.organization||'Fundación Utopía');applyLiveBackground(d.raffle?.liveBackgroundUrl,d.raffle?.liveBackgroundType);renderLiveStats(d.stats||{});renderLivePrizeOptions(d.prizes||[],previousPrize);renderLiveWinners(d.winners||[]);
      qs('#livePrepareBackground').disabled=!state.liveMigrationReady;qs('#liveApplyBackground').disabled=!state.liveMigrationReady||!state.liveBackgroundDraft;qs('#liveFullscreen').disabled=!state.liveMigrationReady;qs('#liveMigrationMessage').textContent=state.liveMigrationReady?'Tickets válidos listos. Cambia el fondo, define en qué salida habrá ganador y luego amplía la pantalla.':'Migra los tickets válidos antes de continuar.';
      if(!state.liveSession){state.liveDigits=firstLiveTicketDigits();state.liveBurned=[];renderLiveReels();renderLiveBurned();qs('#liveRevealDigit').disabled=true;qs('#liveCancelDraw').disabled=true;qs('#liveRevealDigit').textContent='Selecciona el premio e inicia la dinámica';qs('#liveProgress').textContent=state.liveMigrationReady?'Selecciona el premio e inicia la dinámica.':'Esperando la migración de tickets válidos.';qs('#liveControlCode').textContent='';if(!preserveWinner)qs('#liveWinnerCard').hidden=true;}
    }
    ...
    async function syncLiveTickets(){
      const button=qs('#liveSyncTickets');button.disabled=true;try{const d=await api('adminLiveDrawSync',{raffleId:state.liveRaffleId});if(!d.ok)throw new Error(d.error);state.liveSetup=d;state.liveMigrationReady=Boolean(d.migrationReady??((d.stats?.eligibleTickets||0)>0&&(d.stats?.approvedNotSold||0)===0));renderLiveStats(d.stats||{});renderLivePrizeOptions(d.prizes||[],'');renderLiveWinners(d.winners||[]);const m=d.migration||{};qs('#liveMigrationMessage').textContent=`Migración completada: ${m.synchronized||0} actualizados, ${m.alreadySold||0} ya estaban vendidos. ${state.liveMigrationReady?'Configura la salida ganadora, carga el fondo y amplía la pantalla.':'No existen tickets elegibles todavía.'}`;qs('#livePrepareBackground').disabled=!state.liveMigrationReady;qs('#liveApplyBackground').disabled=!state.liveMigrationReady||!state.liveBackgroundDraft;qs('#liveFullscreen').disabled=!state.liveMigrationReady;toast('Tickets válidos sincronizados.');}finally{button.disabled=false;}
    }
    function lockLiveControls(locked){qs('#liveRaffleSelect').disabled=locked;qs('#livePrizeSelect').disabled=locked||!state.liveMigrationReady;qs('#liveWinnerAt').disabled=locked||!state.liveMigrationReady;qs('#liveSyncTickets').disabled=locked;qs('#liveStartDraw').disabled=locked||!state.liveMigrationReady||!qs('#livePrizeSelect').value;qs('#liveCancelDraw').disabled=!locked;qs('#livePrepareBackground').disabled=locked||!state.liveMigrationReady;qs('#liveApplyBackground').disabled=locked||!state.liveMigrationReady||!state.liveBackgroundDraft;}
    async function startLiveDraw(){
      ...
      const d=await api('adminLiveDrawStart',{raffleId:state.liveRaffleId,prizeId,winnerAt});if(!d.ok)throw new Error(d.error);state.liveSession={token:d.drawToken,digitCount:d.digitCount,totalTickets:d.totalTickets||state.liveSetup?.raffle?.totalTickets||1,prize:d.prize,controlCode:d.controlCode,winnerAt:d.winnerAt,current:0};state.liveCurrentWinner=null;state.liveDigits=randomLiveTicketDigits();state.liveBurned=[];renderLiveBurned();renderLiveWinners(state.liveWinners);qs('#liveWinnerCard').hidden=true;lockLiveControls(true);qs('#liveRevealDigit').disabled=false;qs('#liveRevealDigit').textContent='DETENER';qs('#liveProgress').innerHTML=`Tómbola girando para la salida <strong>1 de ${d.winnerAt}</strong> · ${d.eligibleCount} tickets elegibles. Presiona <strong>DETENER</strong>.`;qs('#liveControlCode').textContent=`Código de control: ${d.controlCode}`;startLiveAnimation();toast('Dinámica iniciada.');
    }
    async function revealLiveDigit(){
      ...
      try{
        ...
        if(d.completed){
          ...
          qs('#liveWinnerNumber').textContent=w.ticket;
          qs('#liveWinnerName').textContent=w.fullName;
          qs('#liveWinnerPrize').textContent=`GANADOR — ${w.prizeName}`;
          qs('#liveWinnerPhone').textContent=w.maskedPhone;
          qs('#liveWinnerCity').textContent=w.city?`Departamento / ciudad: ${w.city}`:'';
          qs('#liveCallWinner').href=w.callPhone?`tel:+${w.callPhone}`:'#';
          qs('#liveWhatsAppWinner').href=w.callPhone?`https://wa.me/${w.callPhone}?text=${encodeURIComponent(`Hola ${w.fullName}, te contactamos de Fundación Utopía para informarte que ganaste ${w.prizeName} con el ticket ${w.ticket}.`)}`:'#';
          qs('#liveWinnerCard').hidden=false;
          qs('#liveProgress').innerHTML='<strong>Ganador registrado. Sus datos aparecen en la columna derecha y el ticket queda excluido de los siguientes premios.</strong>';
          toast('¡Ganador registrado!');
          ...
          if(refresh.ok){
            ...
            renderLiveWinners(refresh.winners||[]);
          }
        }else{
          state.liveBurned=d.burnedTickets||[...state.liveBurned,d.ticket];
          renderLiveBurned();
          for(let remaining=10;remaining>0;remaining--){
            if(!state.liveSession||state.liveSession.token!==sessionToken)return;
            button.textContent=`SIGUIENTE NÚMERO EN ${remaining} s`;
            qs('#liveProgress').innerHTML=`Ticket <strong>${esc(d.ticket)}</strong> quemado. La siguiente salida (${d.nextDraw} de ${d.winnerAt}) estará disponible en <strong>${remaining} segundos</strong>.`;
          ...
          qs('#liveProgress').innerHTML=`Tómbola girando para la salida <strong>${d.nextDraw} de ${d.winnerAt}</strong>. Presiona <strong>DETENER</strong>.`;
        }
      }catch(e){
    ...
    async function nextLivePrize(){stopLiveAnimation();state.liveSession=null;state.liveCurrentWinner=null;state.liveBurned=[];lockLiveControls(false);qs('#liveWinnerCard').hidden=true;renderLiveWinners(state.liveWinners);await loadLiveDraw();toast('Selecciona el siguiente premio y define la nueva dinámica.');}
    ...
    function setup(){
      ...
      qs("#adminRaffleSelect").onchange=e=>{state.raffleId=e.target.value;state.publicBackgroundDraft=null;updatePublicPageLink();refreshView(state.currentView,true)};
      ...
      bind("#liveRaffleSelect","change",e=>{state.liveRaffleId=e.target.value;state.liveSession=null;state.liveCurrentWinner=null;state.liveBackgroundDraft=null;qs('#liveBackgroundReady').hidden=true;qs('#liveApplyBackground').disabled=true;stopLiveAnimation();loadLiveDraw()});

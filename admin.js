  ...
  (()=>{
    ...
    const BUILD_VERSION="12.6.0-20260717";
    ...
    async function handleLivePrizeSelection(){
      ...
      if(value&&state.liveCurrentWinner){
        qs('#liveWinnerCard').hidden=true;
        qs('#liveProgress').textContent='Premio seleccionado. Iniciando la siguiente dinámica…';
      }
      syncLivePrimaryButton();
      updateLiveBurnedVisibility();
      ensureLiveVideoPlayback();
      if(value&&!state.liveSession&&!state.liveStarting&&state.liveRunConfirmedRaffleId===state.liveRaffleId){
        await startLiveDraw({skipConfirm:true,auto:true});
      }
    }
    ...
    async function startLiveDraw(options={}){
      ...
      try{
        ...
        updateLiveBurnedVisibility();
        ensureLiveVideoPlayback();
        toast(options.auto?'Siguiente premio iniciado automáticamente.':'Dinámica iniciada.');
      }catch(error){
        stopLiveAnimation();
    ...
    async function revealLiveDigit(){
      ...
      try{
        ...
        if(d.completed){
          ...
          qs('#liveProgress').textContent='Ganador registrado. Selecciona el siguiente premio; la dinámica comenzará automáticamente.';

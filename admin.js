  ...
  (()=>{
    ...
    const state={token:localStorage.getItem("utopiaAdminToken")||"",user:null,raffles:[],raffleId:DEFAULT_RAFFLE,deletedReports:[],currentView:"dashboard",orders:[],participants:[],payments:[],board:[],filteredBoard:[],numberPage:0,selectedNumber:null,results:[],report:null,raffleData:null,publicBackgroundDraft:null,liveRaffleId:DEFAULT_RAFFLE,liveSetup:null,liveSession:null,liveDigits:[],liveBurned:[],liveSpinTimer:null,liveMigrationReady:false,liveBackgroundDraft:null,liveObjectUrl:null,liveCurrentWinner:null,liveWinners:[],loadedAt:{},refreshSequence:0};
    ...
    const BUILD_VERSION="12.3.3-20260715";
    ...
    function updateLivePrizePreview(prizeId=''){
      ...
      if(img)img.src=prize?(prize.imageUrl||prizeFallback(prize)):'assets/rifa-solidaria-2026.png';
    ...
    function renderLivePrizeOptions(prizes,keep=''){
      ...
      if(keep&&available.some(p=>p.id===keep))select.value=keep;
      select.disabled=!state.liveMigrationReady||Boolean(state.liveSession);qs('#liveWinnerAt').disabled=!state.liveMigrationReady||Boolean(state.liveSession);qs('#liveStartDraw').disabled=!state.liveMigrationReady||!select.value||Boolean(state.liveSession);
    ...
    async function loadLiveDraw(preserveWinner=false){
      ...
      if(!state.liveSession){state.liveDigits=firstLiveTicketDigits();state.liveBurned=[];renderLiveReels();renderLiveBurned();qs('#liveRevealDigit').disabled=true;qs('#liveCancelDraw').disabled=true;qs('#liveRevealDigit').textContent='DETENER';qs('#liveProgress').textContent='';qs('#liveControlCode').textContent='';if(!preserveWinner){qs('#liveWinnerCard').hidden=true;const ph=qs('#liveWinnerPlaceholder',false);if(ph)ph.hidden=false;}}
    }
    ...
    async function revealLiveDigit(){
      ...
      try{
        ...
        if(d.completed){
          ...
          qs('#liveWinnerCard').hidden=false;const ph=qs('#liveWinnerPlaceholder',false);if(ph)ph.hidden=true;
    ...
    async function nextLivePrize(){stopLiveAnimation();state.liveSession=null;state.liveCurrentWinner=null;state.liveBurned=[];lockLiveControls(false);qs('#liveWinnerCard').hidden=true;const ph=qs('#liveWinnerPlaceholder',false);if(ph)ph.hidden=false;renderLiveWinners(state.liveWinners);await loadLiveDraw();toast('Listo para seleccionar el siguiente premio.');}
    ...
    function setup(){
      ...
      bind("#livePrizeSelect","change",()=>{const value=qs("#livePrizeSelect").value;qs("#liveStartDraw").disabled=!state.liveMigrationReady||!value;updateLivePrizePreview(value);});

  ...
  (()=>{
    ...
    const state={token:localStorage.getItem("utopiaAdminToken")||"",user:null,raffles:[],raffleId:DEFAULT_RAFFLE,deletedReports:[],currentView:"dashboard",orders:[],participants:[],payments:[],board:[],filteredBoard:[],numberPage:0,selectedNumber:null,results:[],report:null,raffleData:null,publicBackgroundDraft:null,ticketBoardConfig:null,ticketBoardBackgroundDraft:null,liveRaffleId:DEFAULT_RAFFLE,liveSetup:null,liveSession:null,liveDigits:[],liveBurned:[],liveSpinTimer:null,liveMigrationReady:false,liveBackgroundDraft:null,liveObjectUrl:null,liveCurrentWinner:null,liveWinners:[],liveBurnedAll:[],loadedAt:{},refreshSequence:0};
    ...
    const BUILD_VERSION="12.4.4-20260716";
    ...
    async function loadBoard(){const [d]=await Promise.all([api("adminBoard",{raffleId:state.raffleId}),loadTicketBoardConfig()]);if(!d.ok)throw new Error(d.error);state.board=d.tickets||[];filterBoard()}
    ...
    async function loadLiveDraw(preserveWinner=false){
      ...
      qs('#livePrepareBackground').disabled=!state.liveMigrationReady;qs('#liveApplyBackground').disabled=!state.liveMigrationReady||!state.liveBackgroundDraft;qs('#liveFullscreen').disabled=!state.liveMigrationReady;qs('#liveDownloadMigrated').disabled=!state.liveMigrationReady;qs('#liveMigrationMessage').textContent=state.liveMigrationReady?'Tickets válidos listos. Cambia el fondo, define en qué salida habrá ganador y luego amplía la pantalla.':'Migra los tickets válidos antes de continuar.';
      if(!state.liveSession){state.liveDigits=firstLiveTicketDigits();state.liveBurned=state.liveBurnedAll||[];renderLiveReels();renderLiveBurned();const reveal=qs('#liveRevealDigit');reveal.hidden=false;reveal.disabled=true;reveal.textContent='DETENER';qs('#liveCancelDraw').disabled=true;qs('#liveProgress').textContent='';qs('#liveControlCode').textContent='';qs('#liveWinnerCard').hidden=true;const next=qs('#liveSidebarNextPrize',false);if(next)next.hidden=true;}
    }
    ...
    async function syncLiveTickets(){
      ...
      try{const d=await api('adminLiveDrawSync',{raffleId:state.liveRaffleId});if(!d.ok)throw new Error(d.error);state.liveSetup=d;state.liveMigrationReady=Boolean(d.migrationReady??((d.stats?.eligibleTickets||0)>0&&(d.stats?.approvedNotSold||0)===0));renderLiveStats(d.stats||{});renderLivePrizeOptions(d.prizes||[],'');renderLiveWinners(d.winners||[]);const m=d.migration||{};qs('#liveMigrationMessage').textContent=`Migración completada: ${m.synchronized||0} actualizados, ${m.alreadySold||0} ya estaban vendidos. ${state.liveMigrationReady?'Ya puedes descargar la lista migrada y configurar el sorteo.':'No existen tickets elegibles todavía.'}`;qs('#livePrepareBackground').disabled=!state.liveMigrationReady;qs('#liveApplyBackground').disabled=!state.liveMigrationReady||!state.liveBackgroundDraft;qs('#liveFullscreen').disabled=!state.liveMigrationReady;if(downloadButton)downloadButton.disabled=!state.liveMigrationReady;toast('Tickets válidos sincronizados.');}finally{button.disabled=false;}
    }
    ...
    async function startLiveDraw(){
      ...
      const d=await api('adminLiveDrawStart',{raffleId:state.liveRaffleId,prizeId,winnerAt});if(!d.ok)throw new Error(d.error);state.liveSession={token:d.drawToken,digitCount:d.digitCount,totalTickets:d.totalTickets||state.liveSetup?.raffle?.maxTicketNumber||state.liveSetup?.raffle?.totalTickets||1,eligibleTickets:d.eligibleTickets||state.liveSetup?.eligibleTicketNumbers||[],prize:d.prize,controlCode:d.controlCode,winnerAt:d.winnerAt,current:0};state.liveCurrentWinner=null;state.liveDigits=randomLiveTicketDigits();state.liveBurned=state.liveBurnedAll||[];renderLiveBurned();renderLiveWinners(state.liveWinners);qs('#liveWinnerCard').hidden=true;const reveal=qs('#liveRevealDigit');reveal.hidden=false;reveal.disabled=false;reveal.textContent='DETENER';const next=qs('#liveSidebarNextPrize',false);if(next)next.hidden=true;lockLiveControls(true);qs('#liveProgress').textContent='';qs('#liveControlCode').textContent='';startLiveAnimation();toast('Dinámica iniciada.');
    }
    async function revealLiveDigit(){
      ...
      try{
        ...
        if(d.completed){
          ...
          button.hidden=true;
          const immediateWinner={...w,prizeImageUrl:w.prizeImageUrl||prizeFallback({order:w.prizeOrder})};
          const merged=[...state.liveWinners.filter(item=>String(item.prizeId)!==String(w.prizeId)),immediateWinner].sort((a,b)=>Number(a.prizeOrder||0)-Number(b.prizeOrder||0));
          ...
          state.liveBurnedAll=Array.from(new Set([...(state.liveBurnedAll||[]),d.ticket]));state.liveBurned=state.liveBurnedAll;
          renderLiveBurned();
    ...
    async function nextLivePrize(){stopLiveAnimation();state.liveSession=null;state.liveCurrentWinner=null;state.liveBurned=state.liveBurnedAll||[];lockLiveControls(false);qs('#liveWinnerCard').hidden=true;const reveal=qs('#liveRevealDigit');reveal.hidden=false;reveal.disabled=true;reveal.textContent='DETENER';const next=qs('#liveSidebarNextPrize',false);if(next)next.hidden=true;renderLiveWinners(state.liveWinners);await loadLiveDraw();toast('Listo para seleccionar el siguiente premio.');}
    ...
    function setup(){
      ...
      qs("#adminRaffleSelect").onchange=e=>{state.raffleId=e.target.value;state.publicBackgroundDraft=null;state.ticketBoardBackgroundDraft=null;updatePublicPageLink();refreshView(state.currentView,true)};
      ...
      bind("#liveRaffleSelect","change",e=>{state.liveRaffleId=e.target.value;state.liveSession=null;state.liveCurrentWinner=null;state.liveBackgroundDraft=null;state.liveBurnedAll=[];state.liveBurned=[];qs('#liveBackgroundReady').hidden=true;qs('#liveApplyBackground').disabled=true;qs('#liveDownloadMigrated').disabled=true;stopLiveAnimation();loadLiveDraw()});

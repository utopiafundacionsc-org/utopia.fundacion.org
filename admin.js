  ...
  (()=>{
    ...
    const state={token:localStorage.getItem("utopiaAdminToken")||"",user:null,raffles:[],raffleId:DEFAULT_RAFFLE,deletedReports:[],currentView:"dashboard",orders:[],participants:[],payments:[],board:[],filteredBoard:[],numberPage:0,numberPageRaffleId:"",selectedNumber:null,results:[],report:null,raffleData:null,publicBackgroundDraft:null,ticketBoardConfig:null,ticketBoardBackgroundDraft:null,liveRaffleId:DEFAULT_RAFFLE,liveSetup:null,liveSession:null,liveDigits:[],liveBurned:[],liveSpinTimer:null,liveMigrationReady:false,liveBackgroundDraft:null,liveObjectUrl:null,liveCurrentWinner:null,liveWinners:[],liveBurnedAll:[],prizeImageDataCache:{},loadedAt:{},refreshSequence:0};
    ...
    const BUILD_VERSION="12.5.1-20260716";
    ...
    async function loadBoard(options={}){const preservePage=options?.preservePage!==false;const sameRaffle=state.numberPageRaffleId===state.raffleId;const previousPage=sameRaffle&&Number.isFinite(state.numberPage)?state.numberPage:savedNumberPage(state.raffleId);const [d]=await Promise.all([api("adminBoard",{raffleId:state.raffleId}),loadTicketBoardConfig()]);if(!d.ok)throw new Error(d.error);state.board=d.tickets||[];state.numberPageRaffleId=state.raffleId;state.numberPage=preservePage?previousPage:0;filterBoard(false)}
    ...
    async function loadLiveDraw(preserveWinner=false){
      ...
      qs('#livePrepareBackground').disabled=false;qs('#liveApplyBackground').disabled=!state.liveBackgroundDraft;qs('#liveFullscreen').disabled=false;qs('#liveDownloadMigrated').disabled=!state.liveMigrationReady;qs('#liveMigrationMessage').textContent=state.liveMigrationReady?'Tickets válidos listos. Cambia el fondo, define en qué salida habrá ganador y luego amplía la pantalla.':'Migra los tickets válidos antes de continuar.';

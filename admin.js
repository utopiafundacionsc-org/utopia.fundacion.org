  ...
  (()=>{
    ...
    const BUILD_VERSION="12.2.3-20260715";
    ...
    async function revealLiveDigit(){
      ...
      try{
        ...
        if(d.completed){
          ...
          if(!state.liveSession||state.liveSession.token!==sessionToken)return;
          state.liveDigits=state.liveDigits.map(()=>({value:randomDigit(),frozen:false}));
          renderLiveReels();
          startLiveAnimation();
          button.disabled=false;
          button.textContent=`SACAR NÚMERO ${d.nextDraw} DE ${d.winnerAt}`;
          qs('#liveProgress').innerHTML=`Ya puedes sacar el número <strong>${d.nextDraw} de ${d.winnerAt}</strong>.`;
        }
      }catch(e){
    ...
    function reportCsv(){const rows=[["Indicador","Valor"],...reportRows()];download(`reporte-${state.raffleId||'sorteo'}.csv`,rows.map(r=>r.map(c=>`"${String(c??'').replaceAll('"','""')}"`).join(",")).join("\n"),"text/csv;charset=utf-8")}
    function reportExcel(){const rows=reportRows().map(([k,v])=>`<tr><td>${esc(k)}</td><td>${esc(v)}</td></tr>`).join("");download(`reporte-${state.raffleId||'sorteo'}.xls`,`<html><meta charset="utf-8"><table border="1"><tr><th>Indicador</th><th>Valor</th></tr>${rows}</table></html>`,"application/vnd.ms-excel;charset=utf-8")}

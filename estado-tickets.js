(()=>{
  ...
  const driveFileId=(value)=>{const text=String(value||"");for(const re of [/\/d\/([a-zA-Z0-9_-]+)/,/[?&]id=([a-zA-Z0-9_-]+)/]){const match=text.match(re);if(match)return match[1]}return""};
  const resolveVideoUrl=(value)=>{const text=String(value||"").trim();const id=driveFileId(text);return id?`https://drive.usercontent.google.com/download?id=${encodeURIComponent(id)}&export=view&authuser=0`:text};
  const statusMap={
  ...
  function setBackground(board={}){
    ...
    if(type==="VIDEO"&&url){
      ...
      video.hidden=false;
      video.muted=true;video.defaultMuted=true;video.loop=true;video.playsInline=true;video.autoplay=true;
      ...
      video.oncanplay=play;video.onloadeddata=play;video.onerror=()=>{video.hidden=true;document.body.classList.remove("has-board-media")};
      play();
      return;
    }
    video.pause();video.removeAttribute("src");video.load();video.hidden=true;
    image.style.backgroundImage=url?`url("${url.replaceAll('"','%22')}")`:"";
  }
  ...
  function render(data){
    ...
    const tickets=(data.tickets||[]).map((item)=>({number:String(item.number||""),status:normalizeStatus(item.status)}));
  ...
  document.addEventListener("click",()=>{const video=qs("#ticketBoardBackgroundVideo");if(video&&!video.hidden)video.play().catch(()=>{})},{once:true});
  load();
})();

(()=>{
  ...
  function showDrivePreviewFallback(fileId){
    ...
    if(!fileId){document.body.classList.remove("has-board-media");return}
  ...
  function tryNextVideoCandidate(fileId){
    ...
    const frame=qs("#ticketBoardBackgroundFrame");
    clearVideoLoadTimer();
    if(videoCandidateIndex>=videoCandidates.length){showDrivePreviewFallback(fileId);return}
    ...
    video.setAttribute("muted","");video.setAttribute("playsinline","");video.setAttribute("autoplay","");video.setAttribute("webkit-playsinline","");
    video.preload="auto";
    video.src=candidate;
    const loaded=()=>{
      clearVideoLoadTimer();
    ...
    video.onerror=()=>tryNextVideoCandidate(fileId);
    video.load();
    video.play().catch(()=>{});
    videoLoadTimer=setTimeout(()=>{
      if(video.readyState<2)tryNextVideoCandidate(fileId);
    },8000);
  }
  function setBackground(board={}){
    ...
    qs("#ticketBoardBackgroundImage").style.backgroundImage=`url("${url.replaceAll('"','%22')}")`;
  }
